...
public interface RecurringDao {
    @Query("SELECT * FROM recurring WHERE is_active = 1 ORDER BY id ASC")
    ...
    @Query("UPDATE recurring SET is_active = :active, updated_at = :now WHERE id = :id")
    void setActive(long id, boolean active, long now);

    @Query("UPDATE recurring SET last_executed_date = :date, updated_at = :now WHERE id = :id")
...
### `AiSummaryRepository` — Gemini Nano AI 요약 (향후 구현)

> [!info] 향후 구현 예정
> Gemini Nano AI 요약 기능은 현재 스텁으로, 추후 Google AI Edge SDK를 통해 구현 예정이다.
